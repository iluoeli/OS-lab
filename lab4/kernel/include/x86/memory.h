...
#define GDT_BASE(i)	((uint32_t)(gdt[i].base_15_0 + (gdt[i].base_23_16<<16) + (gdt[i].base_31_24 << 24)))


#define MAX_SEM_NUM 32
#define SEM_VALID 1
#define SEM_USED 0

struct Semaphore {
	int value;
	struct ProcessTable *list;	//process blocked on this semaphore
	int state;		//valid when 1, else invalid
	int linkCount;	//link count, this sem will be destroied only when linkCoutn == 1
